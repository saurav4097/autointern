...
export default function WebDevelopment() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const intern = await fetch("/api/auth/get-internship").then(r => r.json());
      const subs = await fetch("/api/auth/get-submissions").then(r => r.json());
        ...
        <div className="max-w-4xl mx-auto space-y-10">
          ...
          {days < 20 && (
            <div className="bg-white p-8 rounded-3xl shadow-xl border border-gray-100">
              ...
              <div className="mt-6 space-y-3">
                {["Setup UI", "Responsive Design", "Deploy Project"].map((t, i) => (
          ...
          {days >= 15 && days < 30 && (
            <div className="bg-white p-8 rounded-3xl shadow-xl border border-gray-100">
              ...
              <div className="mt-6 space-y-3">
                {["Backend API", "Database Integration", "Authentication"].map((t, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <div className="w-6 h-6 rounded-full bg-green-500 text-white flex items-center justify-center text-sm">
                      {i + 1}
                    </div>
